...
class DropdownMenu extends Component {
  ...
  constructor(props, context) {
    ...
    this.defaultConfig = {
      ...
      arrowImg: require('../../assets/dropdown/dropdown_arrow.png'),
      checkImage: require('../../assets/dropdown/menu_check.png')
    };
  ...
  renderChcek(index, title) {
    ...
    if (this.state.selectIndex[activityIndex] == index) {
      ...
      return (
        <View style={{ width: width, justifyContent: 'space-between', alignItems: "center", paddingHorizontal: 15, flexDirection: 'row' }} >
          <Text
            style={[
    ...
    if (this.state.activityIndex >= 0) {
      ...
      return (
        <View style={{ position: 'absolute', left: 0, right: 0, top: 40, bottom: 0 }}>
          ...
          <ScrollView style={[{ position: 'absolute', top: 0, left: 0, right: 0, backgroundColor: 'white', width: 100 }, heightStyle]} >
            {
              currentTitles.map((title, index) =>
                <TouchableOpacity key={index} activeOpacity={1} style={{ width: width, height: 44 }} onPress={this.itemOnPress.bind(this, index)} >
